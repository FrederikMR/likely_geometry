...
class ProbScoreGEORCEFM(ABC):
    def __init__(self,
                 M:RiemannianManifold,
                 score_fun:Callable[[torch.Tensor],torch.Tensor],
...
class ProbScoreGEORCEFM_Embedded(ABC):
    def __init__(self,
                 proj_fun:Callable,
    ...
    def gi(self,
        ...
        gi, Gi = grad(self.inner_product, has_aux=True)(zi, ui)
        gi += self.lam1_norm*self.score_fun(zi)
...
class ProbScoreGEORCEFM_Euclidean(ABC):
    def __init__(self,
                 score_fun:Callable[[torch.Tensor],torch.Tensor],
                 init_fun:Callable[[torch.Tensor, torch.Tensor, int], torch.Tensor]=None,
                 lam:float=1.0,
